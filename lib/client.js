...
/* The message editor still consumes an earlier, unsuffixed DSH token set.
 * Restore those aliases only inside the turn-tail edit slot so its declared
 * border, paper fill and primary action remain intact without affecting other
 * third-party surfaces. */
html[data-dsh-theme-eink-retro]
  [data-slot="conversation.chat.turnTail"]:has(textarea[placeholder="\u7F16\u8F91"]) {
  --dsw-alias-border: var(--eink-rule-strong);
  --dsw-alias-fill-primary: var(--eink-paper-bright);
  --dsw-alias-text-primary: var(--eink-ink);
  --dsw-alias-accent: var(--eink-selection-bg);
  --dsw-alias-text-on-accent: var(--eink-selection-fg);
}

html[data-dsh-theme-eink-retro]
  [data-slot="conversation.chat.turnTail"]:has(textarea[placeholder="\u7F16\u8F91"])
  :has(> textarea[placeholder="\u7F16\u8F91"])
  > :nth-child(2)
  > button:first-child {
  background-color: var(--eink-paper-bright) !important;
}
